...
func TestAccCloudflareR2BucketLock_Update(t *testing.T) {
	rnd := utils.GenerateRandomResourceName()
	accountID := os.Getenv("CLOUDFLARE_ACCOUNT_ID")
	resourceName := "cloudflare_r2_bucket_lock." + rnd
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccR2BucketLockUpdateConfig(rnd, accountID),
				ConfigStateChecks: []statecheck.StateCheck{
					statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("rules"), knownvalue.ListSizeExact(3)),
					statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("rules").AtSliceIndex(0).AtMapKey("condition").AtMapKey("max_age_seconds"), knownvalue.Int64Exact(172800)),
					statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("rules").AtSliceIndex(1).AtMapKey("id"), knownvalue.StringExact("archive-lock")),
					statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("rules").AtSliceIndex(1).AtMapKey("condition").AtMapKey("max_age_seconds"), knownvalue.Int64Exact(604800)),
					statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("rules").AtSliceIndex(2).AtMapKey("id"), knownvalue.StringExact("indefinite-lock")),
					statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("rules").AtSliceIndex(2).AtMapKey("enabled"), knownvalue.Bool(false)),
					statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("rules").AtSliceIndex(2).AtMapKey("condition").AtMapKey("type"), knownvalue.StringExact("Indefinite")),
				},
			},
			{
				ResourceName:                         resourceName,
...
func testAccR2BucketLockUpdateConfig(rnd, accountID string) string {
...
func testAccR2BucketLockDateConditionConfig(rnd, accountID string) string {
	return acctest.LoadTestCase("date_condition.tf", rnd, accountID)
}

func testAccR2BucketLockIndefiniteConditionConfig(rnd, accountID string) string {
	return acctest.LoadTestCase("indefinite_condition.tf", rnd, accountID)
}

func testAccR2BucketLockMinimalRulesConfig(rnd, accountID string) string {
	return acctest.LoadTestCase("minimal_rules.tf", rnd, accountID)
}
